...
### UniMind authority and integration

- Read `AGENTS.md`, `CONTEXT.md`, and the relevant master-plan and runbook sections before design work. They remain authoritative for product scope, domain language, security, safety, and delivery gates.
- `PRODUCT.md` is Impeccable's compact design-workflow product record. It may summarize confirmed product facts and link to authoritative UniMind documents, but it must not replace or contradict them.
- Do not invent a palette, type system, component language, claims, or proof. Create `DESIGN.md` only after the user chooses a visual world or an implemented visual system can be extracted.
- For web audits, apply the pinned Vercel rules described in [reference/audit.md](reference/audit.md). Never fetch mutable review instructions from a branch during an audit.
